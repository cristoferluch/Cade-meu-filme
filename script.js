...
async function loadMovies(movies){
    ...
    const movies_list = document.querySelector(".filmes");
    ...
    const moviePromises = movies.map(async (movie) => {
        if(movie.poster_path != null){
            ...
            movies_list.append(filme)
        }
    })
    // verifica se tudo foi carregado antes de exibir
    try {
        await Promise.all(moviePromises);
        document.querySelector('body').style = "justify-content: flex-start";
        document.querySelector('.filmes').style = "display: grid";
    } catch (error) {
